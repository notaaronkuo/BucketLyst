...
import {ListItemsOthers} from "../ListCardsPage/list-cards";
...
function UserProfile() {
  ...
  const MyProfileDummyData = {
    ...
    2: {
      id: 2,
      name: 'SampleUser2', //This is also stored locally in session storage
    ...
    const sampleuser = MyProfileDummyData[1];
  ...
  console.log('Current route:'+ getCurrentRoute());
    ...
    async function getMyProfileInfo() {
      ...
      const apiUrl = `https://852piontsi.execute-api.us-east-1.amazonaws.com/dev/userProfile?userid=${getCurrentRoute()}`;
  ...
  />
              ...
              <div className="ph-c2">
                <div className="ph-c2r1">
                  ...
                  <div className="User-username">{MyUser[0]?.name ? MyUser[0].name : "No Username"}</div>
          ...
          <ProfileListHolder routeID={MyrouteID} />
        </div>
)}
...
function ProfileListHolder({ routeID }) {
  
  const [Lists, setLists] = useState([]);
  
  async function getListInfo() {
    console.log("Retrieving List Data");
    const apiUrl = `https://z3f03m72db.execute-api.us-east-1.amazonaws.com/dev/viewlist?listType=myList&userid=${routeID}`;
  ...
  if (!Lists || Lists.lengths === 0) {
    return <div classname = "list-holder">No lists available.</div>;
  }
  
  const listings = Lists.map((lists) => (
    <ListItemsOthers key={lists.id} {...lists} />
  ));

  return (
    <>
    {Lists === null ? (<div className='loading-icon-holder'>
  ...
  <div className="profile-listholder">{listings}</div>
          )}
  </>
  );
}
